...
## Multi-Agent GenAI Metrics

This section defines additional metrics for monitoring and evaluating multi-agent GenAI systems. These metrics focus on latency, throughput, cost, resource usage, and safety signals at an aggregated level. They follow OpenTelemetry semantic conventions and use histograms, gauges, and counters where appropriate.

---

### Performance Metrics

#### `gen_ai.performance.latency.user_to_response`
Measures end-to-end latency from user input to final response in multi-agent systems.

- **Instrument Type**: Histogram
- **Unit**: seconds
- **Recommended Aggregation**: Percentiles (p50, p90, p99)
- **Attributes**:
  - `gen_ai.task.type`

---

#### `gen_ai.performance.latency.tool_invocation`
Breakdown of tool-specific invocation latencies within agent spans.

- **Instrument Type**: Histogram
- **Unit**: seconds
- **Recommended Aggregation**: Percentiles
- **Attributes**:
  - `gen_ai.tool.name`

---

#### `gen_ai.performance.throughput.agent_calls`
Number of agent invocations handled per second.
...
#### `gen_ai.performance.throughput.tasks_completed`
Number of tasks completed per minute by the system.
...
#### `gen_ai.performance.resource_utilization`
Tracks CPU and memory usage per agent instance or component.

- **Instrument Type**: Gauge
- **Unit**: percent
- **Recommended Aggregation**: Latest value or average
- **Attributes**:
  - `gen_ai.agent.name`
  - `gen_ai.resource_id`
...
#### `gen_ai.performance.bottlenecks.underutilized`
Tracks agents with consistently low workload or engagement.

- **Instrument Type**: Gauge
- **Unit**: 1 (binary or normalized scale)
- **Recommended Aggregation**: Average
- **Attributes**:
  - `gen_ai.agent.name`
...
#### `gen_ai.performance.bottlenecks.latency_sources`
Identifies components contributing to significant latency.

- **Instrument Type**: Gauge
- **Unit**: milliseconds or normalized scale
- **Recommended Aggregation**: Average or max
- **Attributes**:
  - `gen_ai.component.name`

---

### Cost Metrics

#### `gen_ai.cost.compute_usd`
Estimated compute cost in USD for processing tasks or agent runs.
...
#### `gen_ai.cost.tool_specific_usd`
Tracks cost of using external or internal tools per agent interaction.
